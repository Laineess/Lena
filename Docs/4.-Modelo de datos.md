...
CREATE TYPE rol_usuario AS ENUM ('administrador', 'cocina', 'mesero');

CREATE TABLE usuario (
  id             uuid PRIMARY KEY,
  sucursal_id    uuid REFERENCES sucursal(id),   -- NULL = admin central, alcance global (RF-C-4)
  nombre         text NOT NULL,
  rol            rol_usuario NOT NULL,
  email          text UNIQUE,                    -- solo administrador (RF-A-1)
  password_hash  text,                           -- Argon2id, solo administrador
  pin_hash       text,                           -- Argon2id, mesero y cocina (RF-A-2)
  activo         boolean NOT NULL DEFAULT true,
  created_at     timestamptz NOT NULL DEFAULT now(),
  updated_at     timestamptz NOT NULL DEFAULT now(),

  CONSTRAINT admin_sin_sucursal
    CHECK ((rol = 'administrador' AND sucursal_id IS NULL)
        OR (rol <> 'administrador' AND sucursal_id IS NOT NULL)),
  CONSTRAINT admin_usa_email
    CHECK ((rol = 'administrador' AND email IS NOT NULL AND password_hash IS NOT NULL)
        OR (rol <> 'administrador' AND pin_hash IS NOT NULL))
);
...
CREATE TABLE comanda_detalle (
  ...
  CONSTRAINT enviada_tiene_fecha
    CHECK ((estado = 'borrador' AND enviada_at IS NULL)
        OR (estado <> 'borrador' AND enviada_at IS NOT NULL))
);
