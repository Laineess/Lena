...
| 1.4 | 2026-07-17 | Carlos González Laines | `producto.subcategoria` (texto libre, RF-D-6): la escribe el admin al dar de alta; el catálogo se organiza categoría → subcategoría (migración 0008). |
| 1.5 | 2026-07-17 | Carlos González Laines | `producto_disponibilidad(producto_id, sucursal_id, disponible)` (migración 0009): "se acabó" es **por sucursal**; ausencia de fila = disponible. El global `producto.disponible` queda en desuso para el menú. |
| 1.6 | 2026-07-17 | Carlos González Laines | Se elimina la columna redundante `producto.disponible` (migración 0010). El precio por sucursal (`producto_precio_sucursal`, RF-D-8) ya se aplica en el catálogo con endpoints de gestión. |
| 1.7 | 2026-07-17 | Carlos González Laines | `retiro_caja` (migración 0011, append-only): salidas de efectivo del fondo durante un turno (compra pagada de la caja). Se resta del esperado del corte. Una compra de insumo pagada en efectivo genera compra + gasto + retiro en una sola operación. |
...
CREATE TABLE producto (
  ...
  nombre       text NOT NULL,
  descripcion  text,
  subcategoria text,                             -- texto libre, la escribe el admin (RF-D-6)
  precio_base  numeric(10,2) NOT NULL CHECK (precio_base >= 0),
  -- El "se acabó" (RF-D-7) es POR SUCURSAL: ver producto_disponibilidad.
...
**La disponibilidad ("se acabó") es POR SUCURSAL** (`producto_disponibilidad`, migración 0009): cada sucursal agota distinto. Ausencia de fila = disponible. El menú del mesero/cocina se resuelve con su sucursal; `producto.disponible` (global) quedó en desuso para el menú.
