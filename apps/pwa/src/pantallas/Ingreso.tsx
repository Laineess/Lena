// Ingreso (09 §3). Paso 1: clave de la sucursal (se recuerda en el dispositivo).
// Paso 2: usuario → PIN (3 toques, RNF-U-1). Al cerrar sesión se vuelve al paso
// 2 (misma sucursal), no al paso 1. El dueño/administrador entra por correo.
...
const LARGO_PIN = 6;
const SUC_KEY = 'lena.sucursal'; // recuerda la sucursal del dispositivo
const INPUT =
  'w-full max-w-xs rounded-md border border-carbon-600 bg-carbon-800 px-3 py-3 text-piedra-100 placeholder-piedra-500';

// Marca de la app: el ícono SVG arriba del nombre (menos en el topbar).
function Marca({ nombre = 'Leña', sub }: { nombre?: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <img src="/icon.svg" alt="" className="h-16 w-16 rounded-2xl" />
      <h1 className="text-h1 font-bold text-oro-400">{nombre}</h1>
      {sub && <p className="text-piedra-400">{sub}</p>}
    </div>
  );
}
...
export function Ingreso({ onIngreso }: { onIngreso: (s: DatosSesion) => void }) {
  ...
  // Al abrir: si el dispositivo ya tiene sucursal recordada, reanuda en el paso
  // de elegir usuario (tras cerrar sesión no se vuelve a pedir la clave).
  useEffect(() => {
    const guardada = localStorage.getItem(SUC_KEY);
    if (!guardada) return;
    const { clave: c } = JSON.parse(guardada) as { clave: string };
    setClave(c);
    sucursalPorClave(c)
      .then((r) => {
        guardarLetra(r.letra);
        setSucursal({ nombre: r.nombre, usuarios: r.usuarios });
      })
      .catch(() => localStorage.removeItem(SUC_KEY));
  }, []);
  ...
  async function entrarClave() {
    ...
    try {
      ...
      guardarLetra(r.letra);
      localStorage.setItem(SUC_KEY, JSON.stringify({ clave: clave.trim().toUpperCase() }));
      setSucursal({ nombre: r.nombre, usuarios: r.usuarios });
    } catch {
  ...
  function cambiarSucursal() {
    localStorage.removeItem(SUC_KEY);
    setSucursal(null);
    setSel(null);
    setClave('');
    setError(null);
  ...
  if (modoAdmin) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 p-6">
        <Marca nombre="Leña" sub="Gestión" />
        {error && <p className="font-bold text-rojo-400">{error}</p>}
  ...
  if (!sucursal) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 p-6">
        <Marca sub="Escribe la clave de tu sucursal" />
        {error && <p className="font-bold text-rojo-400">{error}</p>}
        <input
          value={clave}
  ...
  if (!sel) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 p-6">
        <Marca nombre="Leña" sub={sucursal.nombre} />
        {error && <p className="font-bold text-rojo-400">{error}</p>}
        ...
        <button type="button" onClick={cambiarSucursal} className="text-sm text-piedra-500">
          ‹ Cambiar de sucursal
        </button>
      </div>
    );
    ...
    <div className="flex h-full flex-col items-center justify-center gap-6 p-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((d) => (
          <button key={d} type="button" onClick={() => toca(d)} className="tactil h-16 w-20 rounded-md bg-carbon-800 text-2xl font-bold text-piedra-100 active:bg-carbon-700">
            {d}
          </button>
        ))}
        <button type="button" onClick={() => setPin(pin.slice(0, -1))} className="tactil h-16 w-20 rounded-md text-2xl text-piedra-300 active:bg-carbon-800">
          ←
        </button>
        <button type="button" onClick={() => toca('0')} className="tactil h-16 w-20 rounded-md bg-carbon-800 text-2xl font-bold text-piedra-100 active:bg-carbon-700">
          0
        </button>
